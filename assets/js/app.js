...
const handleHeaderState = () => {
  const syncHeaderState = () => {
    document.body.dataset.scrolled = window.scrollY > 18 ? 'true' : 'false';
  };
...
const handleScrollReveal = () => {
  const targets = [
    ...document.querySelectorAll('.content-section, .site-footer, .hero-card, .hero-fact, .story-note, .story-fact, .info-card, .highlight-card, .review-quote, .contact-card, .map-card, .faq-item, .media-card'),
  ];
